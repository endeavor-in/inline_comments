  ...
  Drupal.inline_comment = function(context) {
    ...
    if(allow_inline_comment == 1 && allow_comment == 2) {
      $('#content .field-item').click(function(e){
        $('#add-inline-comment-image').remove();
        if(!(comment_form_visible)) {
          ...
          if(window.getSelection() != '') {
            ...
            if (textRange.toString()) {
              ...
              $('#page-wrapper').after("<div id='add-inline-comment-image'><img title='Add a comment for selected text' src='../sites/all/modules/inline_comments/comment_add.png' style='position:absolute; left:" + left + "px; top:" + top + "px'/></div>");
